# Claude Code Project Instructions — clap-kotlin

## Project Overview

This is **clap-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`clap`](https://crates.io/crates/clap).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/clap/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/clap>
