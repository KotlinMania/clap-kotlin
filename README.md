...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`clap`](https://crates.io/crates/clap).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `clap` crate will be cloned into `tmp/` (gitignored) when porting begins.
...
Original work copyrighted by the upstream `clap` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
